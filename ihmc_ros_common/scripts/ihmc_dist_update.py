...
def main():
    ...
    ihmcCommonDir = rospack.get_path('ihmc_ros_common')

    currentVersion = 0

    for f in os.listdir(ihmcCommonDir):
        if "IHMCAtlasAPI" in f and os.path.isdir(os.path.join(ihmcCommonDir, f)):
            currentVersion = semantic_version.Version(f.split("IHMCAtlasAPI-")[1])

    if latestVersion > currentVersion:
        print "Update Available! Preparing to download " + newTar
        updateDistribution(newTar, bucket_url, ihmcCommonDir)
    else:
...
def updateDistribution(newTarName, bucket_url, ihmcCommonDir):
    for f in os.listdir(ihmcCommonDir):
        if "IHMCAtlasAPI" in f and os.path.isdir(os.path.join(ihmcCommonDir, f)):
            sys.stderr.write('WARNING: Found ' + f + ' at ihmc_ros_common/' + f + '. \n')
            ...
            shutil.rmtree(os.path.join(ihmcCommonDir, f))


    destFile = os.path.join(ihmcCommonDir, newTarName)

    print "Downloading " + newTarName + " to ihmc_ros_common package..."
    ...
    archiveHandle.extractall(ihmcCommonDir)
    os.remove(destFile)
